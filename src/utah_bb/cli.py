...
from utah_bb.organize_maps import MapUnit, create_map_unit
...
def see_compactness(state:str, mtype:str):
    ...
    text = click.wrap_text(f"{state}'s {mtype} map had an avg. reock (compactness score) of {avg_reock}. 0 is not compact; 1 is optimally compact.")
    click.echo(text)
...
def compare_proposed_enacted(state:str):
    ...
    text = click.wrap_text(f'Compactness of proposed map: {proposed_reock} v. compactness of enacted map: {enacted_reock} (Remember 1 = optimally compact).')
    click.echo(text)
