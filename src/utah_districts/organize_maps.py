...
class MapUnit:
    ...
    def __init__(
        self, state: str, mtype: str, map_name: str, map_path: str, score_path: str
    ):
        if state not in self._VALID_STATES:
            raise NotImplementedError(
                f"{state} not implemented. Must be one of {','.join(self._VALID_STATES)}"
            )
        if mtype not in self._VALID_TYPES:
            raise NotImplementedError(
                f"{mtype} not implemented. Must be one of {','.join(self._VALID_TYPES)}"
            )
        self.state = state
        self.map_type = mtype
